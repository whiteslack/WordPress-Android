...
import kotlinx.android.synthetic.main.home_page_picker_fragment.previewTypeSelectorButton
...
class HomePagePickerFragment : Fragment() {
    ...
    private fun showModeSelector() {
        previewTypeSelectorButton.post(Runnable {
            ...
            previewModeSelector.setAdapter(
                    PreviewModeMenuAdapter(
                            requireActivity(),
                            if (viewModel.mobileThumbnails.value == true) PreviewMode.DEFAULT else PreviewMode.DESKTOP
                    )
            )
            ...
            previewModeSelector.setOnItemClickListener { parent, _, position, _ ->
                ...
                val current = if (viewModel.mobileThumbnails.value == true) PreviewMode.DEFAULT else PreviewMode.DESKTOP
                if (selectedMode != current) {
                    viewModel.onThumbnailModeChanged(selectedMode == PreviewMode.DEFAULT)
                }
            }
